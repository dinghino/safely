...
import type { Column, ColumnDataType, ColumnOptionExtended, FilterModel } from '../core/types'
import { numberFilterOperators } from '../core/operators'

import { take } from '../lib/array'
import { createNumberRange, formatDateRange, hasOneItem, isEmptyArray } from '../lib/helpers'
...
export const FilterValue = memo(FilterValue__Internal) as typeof FilterValue__Internal

function FilterValue__Internal<TData, TType extends ColumnDataType>(
...
export function FilterValueOptionDisplay<TData>(props: FilterValueDisplayProps<TData, 'option'>) {
  ...
  return (
    <div className="inline-flex items-center gap-0.5">
      {hasOptionIcons &&
        take(selected, 3).map(({ value, icon }, i) => {
          const Icon = icon!
          return isValidElement(Icon) ? Icon : <Icon key={value ?? i} className="size-4" />
        })}
...
export function FilterValueDateDisplay<TData>(props: FilterValueDisplayProps<TData, 'date'>) {
  ...
  if (isEmptyArray(filter.values)) return <Ellipsis className="size-4" />
  if (hasOneItem(filter.values)) {
...
export function FilterValueTextDisplay<TData>(props: FilterValueDisplayProps<TData, 'text'>) {
  ...
  if (!filter) return null
  if (!hasOneItem(filter.values) || filter.values[0].trim() === '')
...
export const FilterValueController = memo(
  FilterValueController__Internal,
) as typeof FilterValueController__Internal

function FilterValueController__Internal<TData, TType extends ColumnDataType>(
    ...
    >
      <div className="flex w-full items-center justify-start gap-1.5">
        ...
        <Badge
          ...
          className={cn(
            count == null && 'hidden',
            'ml-0.5 text-xs tabular-nums tracking-tight',
            ...
            >
              ...
              <TabsContent value="single" className="mt-4 flex flex-col gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {/** biome-ignore lint/correctness/useUniqueElementIds: library requirements */}
                  <DebouncedInput
                    id="single"
                    type="number"
