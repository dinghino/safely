/**
 * imported from https://github.com/sersavan/shadcn-multi-select-component
 */
/** biome-ignore-all lint/nursery/useSortedClasses: will sort later */
/** biome-ignore-all lint/a11y/useSemanticElements: imported component, html conflicts with button in button */
...
interface MultiSelectOption {
  ...
  disabled?: boolean
  /** Additional search keywords (not displayed, only for filtering) */
  keywords?: string[]
  ...
  style?: {
    ...
    gradient?: string
    /** Border color (alternative to badgeColor for outline style) */
    borderColor?: string
    /** Border width when using borderColor */
    borderWidth?: string
  }
}
  ...
  (
    ...
    const isGroupedOptions = React.useCallback(
      (opts: MultiSelectOption[] | MultiSelectGroup[]): opts is MultiSelectGroup[] => {
        return opts && opts.length > 0 && 'heading' in opts[0]!
      },
      [],
    )
    ...
    const filteredOptions = React.useMemo(() => {
      ...
      const filterFn = (option: MultiSelectOption) => {
        const { label, value, keywords } = option
        const comparison = [label, value, ...(keywords ?? [])].map((item) => item.toLowerCase())
        const isMatch = comparison.some((item) => item.includes(searchLower))
        console.log(isMatch, searchLower, comparison)
        return isMatch
      }

      if (isGroupedOptions(options)) {
        return options
          .map((group) => ({ ...group, options: group.options.filter(filterFn) }))
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              ...
                              className="-my-0.5 ml-1 -mr-2 flex cursor-pointer items-center justify-center rounded-sm p-1 hover:bg-primary/20 focus:outline-none focus:ring-1 focus:ring-primary/50"
                            >
                              <XCircle
                                className={cn(
                                  'h-3 w-3',
